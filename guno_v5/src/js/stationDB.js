/* =========================
   GUNO v5 stationDB.js
   - ここにDB/定数を集約（window.GUNO にぶら下げる）
   - v4.05 の STATIONS_DB / TEIDEN_FILES を移植
   - 画像URL等もここに寄せる
   ========================= */
...
(function () {
  ...
  // ---- Assets ----
  GUNO.IMAGE_BASE_URL = "https://geodo.earth/guno_v2/cards/";
  GUNO.BACK_URL = GUNO.IMAGE_BASE_URL + "GUNO_BACK.png";

  // ---- Stations DB (Game Core) ----
  // NOTE:
  // - lc: line code ("JY","M","G","T")
  // - order: 1..10
  // - st_ja / st_en: 表示駅名（★あり）
  // - file: （従来画像カードのファイル名）互換のため残している
  GUNO.STATIONS_DB = [
    // --- JY ---
  ...
  // ---- Special cards ----
  GUNO.TEIDEN_FILES = { JY: "JY_TEIDEN", M: "M_TEIDEN", G: "G_TEIDEN", T: "T_TEIDEN" };

  // ---- Helpers (optional but convenient) ----
  GUNO.normStar = (s) => (s || "").replaceAll("★", "");

  GUNO.getLineMeta = (lc) => {
    const row = GUNO.STATIONS_DB.find((x) => x.lc === lc);
    if (!row) return null;
    return { lc, name_ja: row.name_ja, name_en: row.name_en, color: row.color };
  };

  GUNO.findStation = (lc, order) => {
    return GUNO.STATIONS_DB.find((x) => x.lc === lc && x.order === order) || null;
  };

  // For future packs (全国/世界) — V5.1以降でここに pack 構造を入れていける
  GUNO.PACKS = {
    tokyo_v1: {
      id: "tokyo_v1",
      label_ja: "東京（4路線×10駅）",
      label_en: "Tokyo (4 lines × 10)",
      lines: ["JY", "M", "G", "T"]
    }
  };
})();