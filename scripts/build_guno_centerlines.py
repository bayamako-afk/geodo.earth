...
- stations_dir: station points, EPSG:4326
  - If a station file matching route stem exists, it is used.
  - Otherwise, all station files in stations_dir are used (union).
...
4) For each resulting segment cluster:
   - Choose two longest LineStrings as "left/right" candidates.
   - Sample points along the longer line, nearest-point match on the other line.
   - Midpoints form the centerline.
   - Simplify (default 5m) and densify smoothing-ish.
5) Merge resulting centerlines, project back to EPSG:4326, write GeoJSON.
...
def project_to_metric(gdf: gpd.GeoDataFrame) -> Tuple[gpd.GeoDataFrame, Optional[str]]:
    """Project EPSG:4326 (or other geographic) to auto UTM; return (projected_gdf, metric_crs_str)."""
    if gdf.crs is None:
        return gdf, None
...
def read_stations_for_route(stations_dir: Path, route_stem: str) -> Tuple[Optional[gpd.GeoDataFrame], str]:
    """
    Station file naming you have:
      tokyo-metro-ginza_stations.geojson
    So we try:
      1) <route_stem>_stations.geojson   (best)
      2) <route_stem>.geojson
      3) contains match
      4) all_union fallback
    Returns (stations_gdf, used_mode)
    used_mode: exact_suffix | exact | contains | all_union | none
    """

    # 1) exact with "_stations" suffix (your convention)
    fp1 = stations_dir / f"{route_stem}_stations.geojson"
    if fp1.exists():
        st = gpd.read_file(fp1)
        return st, "exact_suffix"

    # 2) exact stem match (just in case)
    fp2 = stations_dir / f"{route_stem}.geojson"
    if fp2.exists():
        st = gpd.read_file(fp2)
        return st, "exact"

    # 3) contains match (robust fallback)
    contains = [p for p in stations_dir.glob("*.geojson") if route_stem in p.stem]
    if len(contains) >= 1:
        contains.sort(key=lambda p: len(p.stem))  # usually most specific first
        st = gpd.read_file(contains[0])
        return st, "contains"
    ...
    return st_all, "all_union"


def stations_union_buffer(stations_m: gpd.GeoDataFrame, buffer_m: float) -> Optional[object]:
...
def cut_station_areas(lines_m: gpd.GeoDataFrame, station_buf_union) -> gpd.GeoDataFrame:
    """
    Remove portions inside station buffer.
    Result can become MultiLineString; explode again.
    """
...
def sample_midline(a: LineString, b: LineString, n: int = 200) -> Optional[LineString]:
    """
    Build midline by sampling points along a and matching nearest on b.
    """
    ...
    pts = []
    # sample n points uniformly by normalized distance
    for i in range(n + 1):
        t = i / n
    ...
    # de-duplicate consecutive near-identical points
    if len(pts) < 2:
        return None
    cleaned = [pts[0]]
    for x, y in pts[1:]:
        px, py = cleaned[-1]
        if (x - px) ** 2 + (y - py) ** 2 > 0.25:  # >0.5m move
            cleaned.append((x, y))
...
def best_two_longest(lines: List[LineString]) -> Optional[Tuple[LineString, LineString]]:
    ...
    a = sorted_lines[0]
    # choose b that is not essentially identical and overlaps spatially
    for b in sorted_lines[1:]:
        if b.length <= 0:
            continue
        # quick overlap-ish check: distance between centroids not huge
...
def process_route(
    ...
    if gdf.crs is None:
        # Assume EPSG:4326 per user statement, but we avoid silently overriding:
        gdf = gdf.set_crs(epsg=4326)

    # explode to LineString parts
    lines = explode_lines(gdf)

    # stations
    st = read_stations_for_route(stations_dir, route_stem)
    if st is not None:
        if st.crs is None:
            st = st.set_crs(epsg=4326)
    else:
        # no stations found => proceed without cutting
        st = None

    # project to metric
    lines_m, metric_crs = project_to_metric(lines)
    if metric_crs is None:
        return {"route": route_stem, "file": str(line_fp), "status": "NO_CRS_METRIC"}
    ...
    # Merge nearby parts into a single MultiLineString then explode again
    merged = unary_union(list(lines_cut.geometry))
    merged = linemerge(merged) if merged is not None else None

    # Flatten merged to list of LineString candidates
    ...
    if merged is None:
        ...
        # fallback: just use original cut lines
    ...
    # If we still have many small pieces, keep only reasonably long ones
    if len(candidates) > 10:
        # keep top 20 by length
        candidates = sorted(candidates, key=lambda g: g.length, reverse=True)[:20]

    pair = best_two_longest(candidates)
    if pair is None:
        return {"route": route_stem, "file": str(line_fp), "status": "NOT_ENOUGH_TRACKS_AFTER_CUT"}

    a, b = pair
    mid = sample_midline(a, b, n=sample_n)
    if mid is None:
        return {"route": route_stem, "file": str(line_fp), "status": "MIDLINE_FAILED"}
    ...
    return {
        ...
        "used_station_file": (str(stations_dir / f"{route_stem}.geojson") if st is not None else None),
        "out_gdf": out_4326
    }
