...
def merge_seed(cluster: List[LineString]) -> LineString:
    u = unary_union(cluster)
    m = linemerge(u)
    # LineString を期待。Multiなら一番長いのを代表に
    if m is None:
        return LineString()
    if m.geom_type == "LineString":
        return m
    if m.geom_type == "MultiLineString":
        geoms = sorted(list(m.geoms), key=lambda g: g.length, reverse=True)
        return geoms[0]
    return LineString()

def cluster_ab(segs: List[LineString], iters: int = 6) -> Tuple[List[LineString], List[LineString], LineString, LineString]:
...
def process_route(line_fp: Path, simplify_m: float, seg_min_m: float, cluster_iters: int, sample_n: int) -> Dict:
    ...
    mid = sample_midline(oneA, oneB, n=sample_n)
    if mid is None or mid.length < 500:
        return {"route": route, "file": str(line_fp), "status": "MIDLINE_FAILED", "metric_crs": metric_crs,
                "lenA_m": float(oneA.length), "lenB_m": float(oneB.length)}
