...
def process_route(line_fp: Path, simplify_m: float, seg_min_m: float, cluster_iters: int, sample_n: int) -> Dict:
    ...
    for a in partsA:
        ...
        best_d = 1e18
        for b in partsB:
            # Use minimum geometry-to-geometry distance (more robust than centroid distance)
            d = a.distance(b)
            if d < best_d:
                best_d = d
                best_b = b

        # Exclude obviously bad pairs, but keep this permissive for metro data.
        # Typical parallel-track spacing is ~5–50m; allow larger to survive gaps.
        if best_b is None or best_d < 0.1 or best_d > 600:
            continue
    ...
    if not mids:
    # Fallback: take the longest component from each cluster and try once.
    a_long = max(partsA, key=lambda g: g.length, default=None)
    b_long = max(partsB, key=lambda g: g.length, default=None)
    if a_long is not None and b_long is not None:
        mid_try = sample_midline(a_long, b_long, n=sample_n)
        if mid_try is not None and mid_try.length > 500:
            mids = [mid_try]
    if not mids:
        return {
            "route": route,
